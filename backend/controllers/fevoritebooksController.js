...
export const createFavoriteBook = async (req, res) => {
    try {
      ...
      // Correct way to check if product exists in favorites
      ...
      console.log("Error in createFavoriteBook:", error);
      res.status(500).json({
        success: false,
...
export const getFavoriteBooks = async (req, res) => {
  try {
    const favorites = await FavoriteBook.find({ user: req.user._id }).populate(
      "product"
    );
